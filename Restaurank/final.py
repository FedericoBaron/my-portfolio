...
def chapter2():
    ...
    for i in range(1,length+1):
        ...
        num[i-1].grid(row = i-1, column = 0)
        root.columnconfigure(0, weight = 1)
        root.rowconfigure(i-1, weight = 1)

    restaurant = []
    for i in range(1, length+1):
        restaurant[i]  = Label(root, updatedmaster[i][1], bg = "black", fg = "gold")
    ...
    # button1 = Button(bottomFrame, text = "Return to homepage", command = lambda: [forgetfunc(length, num), chapter1()])
